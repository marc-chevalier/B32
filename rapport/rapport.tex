...
L'algorithme qui renvoit les 4 bits les plus probables pour la sous-clé qui nous intéresse est donné ci-dessous. Dans cet algorithme, nous ne devinons que 4 bits pour les deux méthodes. Nous expliquerons ce choix à la question 8.
...
\caption{L'attaque des 4 bits de la clé pouvant être devinés}
	...
	\Pour{chaque couple $(a,b)$ du bon type (une ou deux actives boxes)}
	{
...
Dans toute cette table, les bits de $m$ seron notés m = $m_0 m_1\cdots m_n$, et les bits de $x_1$ (que l'on renomme $x$) seront notés $x_1 = x_0 x_1 \cdots x_n$. On fera notamment attention au fait que, dans ce tableau, $x_1$ désignera le deuxième bit de $x$ fraîchement renommé.
...
En pratique, nous avons utilisé des deux actives boxes, mais avec un algorithme qui ne devine que des blocs de 4 bits correspondant précisement aux bits sur lesquels les relations intervenants dans la table de la question 6 opèrent.
